...
var Logger = new Class({
	...
	***/
	initialize: function() {
		...
		this.LOG.id = "logger";
		this.LOG.set('style', 'width: 100%; font-size: 90%; border: 1px solid #000;');
	...
	***/
	trace: function() {
		...
		for(var u=0; u<stackLength; u++) this.log(stack[u]);
...
function _exec(line) {
	...
	try{ 
		...
		sensis.warn(err + ' @ line ' + err.line); 
		if($defined(err.sourceURL)) sensis.warn(err.sourceURL); 
	}
}
