...
var Logger = new Class({
	...
	***/
	_cmd: function() {
		...
		$('runButton').addEvent('click', function() { _exec($('cmd').value); return false;	}); 	
	 	$('sourceButton').addEvent('click', function() { _viewSource(); return false; }); 	
	 	$('clearButton').addEvent('click', function() {	_clear(); return false;	});
	 	$('logButton').addEvent('click', function() { _log2server(); return false; });
	}, 
	...
	traverseNode: function(elementObj, nodeStack) {
		...
		if(nodeAttributesLength > 0) {
			for(n=0; n<nodeAttributesLength; n++)
				// force IE not to print empty attributes
				if(node.attributes[n].nodeValue != "" && node.attributes[n].nodeValue != null)
...
function _exec(line) {
	...
	catch(err) { sensis.warn(err.name + ': ' + err.message); }
}
